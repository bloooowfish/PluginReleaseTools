...
$RepoOwner = $ReleaseConfig.Owner
$RepoName = $ReleaseConfig.RepoName
$ReleaseVersionPattern = '^\d+\.\d+\.\d+\.\d+$'
$RepoRoot = $ReleaseConfig.RepoRoot
$ProjectPath = $ReleaseConfig.ProjectPath
$BuiltZipPath = $ReleaseConfig.BuiltZipPath
$DistDir = $ReleaseConfig.DistDir
...
function Assert-TagAvailable {
    ...
    $remoteTag = & git -C $RepoRoot ls-remote --tags origin "refs/tags/$TagName"
    if ($LASTEXITCODE -ne 0) {
...
$tagName = "v$Version"
$assetName = $ReleaseConfig.ReleaseAssetName -f $Version
$releaseTitle = $ReleaseConfig.ReleaseTitle -f $Version
$releaseNotes = $ReleaseConfig.ReleaseNotes -f $Version

Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'checkout', 'main')
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'pull', '--ff-only', 'origin', 'main')
Assert-TagAvailable -TagName $tagName
Assert-GitHubReleaseAvailable -TagName $tagName
Set-ProjectVersion -Value $Version

if ($ReleaseConfig.TestProjectPath) {
    Invoke-Checked -FilePath 'dotnet' -Arguments @('test', $ReleaseConfig.TestProjectPath, '-c', 'Debug', '--no-restore')
}
...
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'config', 'user.name', $RepoOwner)
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'config', 'user.email', $ReleaseConfig.ExpectedGitUserEmail)
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'add', '--', $ProjectPath)
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'commit', '-m', "release: $Version")
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'tag', $tagName)
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'push', 'origin', $tagName)
...
Invoke-Checked -FilePath 'git' -Arguments @('-C', $RepoRoot, 'push', 'origin', 'main')

Write-Host "Released $releaseTitle from GitHub Actions."
